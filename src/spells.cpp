...
void _whipEffect(levelManager_t& _mng, spell_t& _spell) {
	...
	if (!(_mng.level.player.isTurnedRight)) {
	...
	_mng.level.playerProjectiles.push_back(whipStrike);
...
void _whipEffect2(levelManager_t& _mng, spell_t& _spell) {
	...
	_mng.level.playerProjectiles.push_back(whipStrike);
	_mng.level.playerProjectiles.push_back(whipStrike2);
...
void _swordEffect(levelManager_t& _mng, spell_t& _spell) {
	projectile_t sweep{};
	auto& playerHitbox = _mng.level.player.hitbox;
	auto playerDir = _mng.lastPlayerDir;
	...
	sweep.sprite = _mng.gameData->projectileData[PROJECTILE_TYPE_SWORD_Y];
	...
	if (playerDir[0] > 0) {
		sweep.hitbox.position[0] += _mng.level.player.hitbox.size[0] + 2.0f;
	}
		...
		sweep.sprite.texPos = _mng.gameData->projectileData[PROJECTILE_TYPE_SWORD_X].texPos;

		sweep.hitbox.position[1] += _mng.level.player.hitbox.size[1] + 2.0f;
		sweep.sprite.texPos[1] += sweep.sprite.texSize[1];
		sweep.sprite.texSize[1] *= -1;
	}
	else {
		std::swap(sweep.hitbox.size[0], sweep.hitbox.size[1]);
		sweep.sprite = _mng.gameData->projectileData[PROJECTILE_TYPE_SWORD_X];
		sweep.sprite.size = sweep.hitbox.size;
	...
	_mng.level.playerProjectiles.push_back(sweep);
...
void _bowEffect(levelManager_t& _mng, spell_t& _spell) {
	...
	arrow.sprite = _mng.gameData->projectileData[PROJECTILE_TYPE_ARROW];
	...
	_mng.level.playerProjectiles.push_back(arrow);
...
void _fireOrbEffect(levelManager_t& _mng, spell_t& _spell) {
	...
	flame.hitbox.position = { playerHitbox.position[0] + _mng.level.player.hitbox.size[0] / 2.0f, playerHitbox.position[1] };
	flame.hitbox.size = { 8, 8 };
	flame.lifetime = 5;
	flame.dmg = (50 + _spell.level * 5);

	flame.spritePosition = flame.hitbox.position;
	flame.sprite = _mng.gameData->projectileData[PROJECTILE_TYPE_FLAME];
	flame.sprite.size = flame.hitbox.size;

	flame.velocity = _mng.closestDir;
	
	normalizeVec(flame.velocity);
	flame.velocity[0] *= flameSpeed;
	flame.velocity[1] *= flameSpeed;

	_mng.level.playerProjectiles.push_back(flame);
